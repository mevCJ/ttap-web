...
class SaveTimetable extends masterState_1.MasterStateAction {
    ...
    GenerateNewState(state) {
        ...
        const currentTimetable = new timetable_1.Timetable(x.FiltrateTimetables[x.CurrentIndex].ListOfSlotUids[x.CurrentSubIndex], x.FiltrateTimetables[x.CurrentIndex].DayTimeMatrix);
        if (currentTimetable) {
