...
function FindTimetable(input, disableClashChecking = false, visualizer) {
    ...
    if (input.length === 1) {
        ...
        return [new timetable_1.Timetable(input[0].SlotIds, resultMatrix)];
    }
    ...
    while (true) {
        ...
        if (disableClashChecking || !matrix_1.GotIntersection(current.DayTimeMatrix, prevSnapshot.DayTimeMatrix)) {
            ...
            if (ptr === last) {
                result.push(new timetable_1.Timetable(snapshots[ptr + 1].SlotIds, snapshots[ptr + 1].DayTimeMatrix));
